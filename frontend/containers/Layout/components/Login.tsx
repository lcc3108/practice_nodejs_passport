...
import { LOGIN, SIGNUP, VALIDATE_ID } from "@/lib/query";
...
import { withApolloClient, ICacheType } from "@/lib/apollo.tsx";
...
class AppBarDialog extends React.Component<ILoginProps, ILoginState> {
  ...
  private async idCheck(userId: string) {
    ...
    const {
      data: { validateId },
    } = await this.props.client.query({ query: VALIDATE_ID, variables: { userId: this.inputId } });
    if (validateId) {
